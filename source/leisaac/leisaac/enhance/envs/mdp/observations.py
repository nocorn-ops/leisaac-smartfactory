...
def _as_tensor(value: object) -> torch.Tensor:
    """把 ``asset.data.*`` / ``sensor.data.*`` 的取值统一成 ``torch.Tensor``。

    - IsaacLab 2.x：这些属性本来就是 ``torch.Tensor``。
    - IsaacLab 3.0+：改为 ``isaaclab.utils.warp.ProxyArray``，需要 ``.torch`` 取回；
      直接传给 isaaclab 的 ``math_utils`` 会报 "Expected a value of type 'Tensor' ... found 'ProxyArray'"。
    """
    if isinstance(value, torch.Tensor):
        return value
    torch_attr = getattr(value, "torch", None)
    if torch_attr is not None:
        return torch_attr if isinstance(torch_attr, torch.Tensor) else torch.as_tensor(torch_attr)
...
def ee_frame_state(
    ...
    # IsaacLab 3.0 起 asset.data.* 返回 ProxyArray（不再是 torch.Tensor），
    # 直接喂给 isaaclab 的 math_utils 会报 "Expected Tensor but found ProxyArray"。
    # 官方迁移方式：用 ProxyArray.torch 取回 tensor（注意 3.0 的四元数是 XYZW）。
    robot_root_pos = _as_tensor(robot.data.root_pos_w)
    robot_root_quat = _as_tensor(robot.data.root_quat_w)
    ee_frame: FrameTransformer = env.scene[ee_frame_cfg.name]
    ee_frame_pos = _as_tensor(ee_frame.data.target_pos_w)[:, 0, :]
    ee_frame_quat = _as_tensor(ee_frame.data.target_quat_w)[:, 0, :]
