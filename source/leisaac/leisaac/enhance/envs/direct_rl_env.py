...
def _has_gui(sim: Any) -> bool:
    """查询是否有 GUI，兼容 IsaacLab 2.x 与 3.0+。

    - IsaacLab 2.x：``sim.has_gui()``（方法）
    - IsaacLab 3.0+：``sim.has_gui``（property，直接当属性用；当方法调用会报
      "'bool' object is not callable"）
    """
    has_gui = getattr(sim, "has_gui", None)
    return bool(has_gui() if callable(has_gui) else has_gui)


def _has_rtx_sensors(sim: Any) -> bool:
    """查询仿真是否带 RTX 传感器（相机），兼容 IsaacLab 2.x 与 3.0+。

    - IsaacLab 2.x：``sim.has_rtx_sensors()``
    - IsaacLab 3.0+：该方法被移除，改为读取 setting ``/isaaclab/render/rtx_sensors``
      （见 isaaclab/envs/direct_rl_env.py 里 ``self.has_rtx_sensors = self.sim.get_setting(...)``）。
    """
    has_rtx = getattr(sim, "has_rtx_sensors", None)
    if callable(has_rtx):
        return bool(has_rtx())
    try:
        return bool(sim.get_setting("/isaaclab/render/rtx_sensors"))
    except Exception:  # noqa: BLE001 —— 查询失败时当作"无相机"，只影响渲染等待逻辑
        return False
...
class RecorderEnhanceDirectRLEnv(DirectRLEnv):
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        is_rendering = _has_gui(self.sim) or _has_rtx_sensors(self.sim)
        ...
        if len(reset_env_ids) > 0:
            ...
            if _has_rtx_sensors(self.sim) and self.cfg.rerender_on_reset:
                self.sim.render()
    ...
    def reset(self, seed: int | None = None, options: dict[str, Any] | None = None) -> tuple[VecEnvObs, dict]:
        ...
        if self.cfg.wait_for_textures and _has_rtx_sensors(self.sim):
            # 惰性导入 + 可选：isaacsim.core.simulation_manager 扩展在 Isaac Sim 6.0 里默认未启用，
            # 导入会 ModuleNotFoundError。这里只是"等纹理加载完"的优化，拿不到就跳过。
            try:
