...
def convert_leisaac_action_to_lerobot(action: torch.Tensor | np.ndarray) -> np.ndarray:
    ...
    Supports single-arm (6 DOF) and bi-arm (12 DOF) actions.
    """
    ...
    for offset in range(0, num_joints, 6):
        for idx, joint_name in enumerate(joint_limits):
            ...
            joint_degree = action[:, actual_idx] - joint_limit_range[0]
            processed_action[:, actual_idx] = joint_degree / joint_range * motor_range + motor_limit_range[0]
...
def convert_lerobot_action_to_leisaac(action: torch.Tensor | np.ndarray) -> np.ndarray:
    ...
    for offset in range(0, num_joints, 6):
        for idx, joint_name in enumerate(joint_limits):
            ...
            motor_range = motor_limit_range[1] - motor_limit_range[0]
            joint_range = joint_limit_range[1] - joint_limit_range[0]
            motor_degree = action[:, actual_idx] - motor_limit_range[0]
            ...
            processed_action[:, actual_idx] = processed_radius
