...
import numpy as np
import torch
from leisaac.utils.constant import BI_ARM_JOINT_NAMES, SINGLE_ARM_JOINT_NAMES
...
class LeRobotServicePolicyClient(Policy):
    ...
    def __init__(
        ...
        if task_type == "so101leader":
            ...
            lerobot_features["observation.state"] = {
                "dtype": "float32",
                "shape": (12,),
                "names": [f"{joint_name}.pos" for joint_name in BI_ARM_JOINT_NAMES],
            }
            self.last_action = np.zeros((1, 12))
    ...
    def _send_observation(self, observation_dict: dict):
        ...
        elif self.task_type == "bi-so101leader":
            left_joint_pos = convert_leisaac_action_to_lerobot(observation_dict["left_joint_pos"])
            right_joint_pos = convert_leisaac_action_to_lerobot(observation_dict["right_joint_pos"])
            for joint_name in BI_ARM_JOINT_NAMES:
                if joint_name.startswith("left_"):
                    short_name = joint_name[5:]  # strip "left_" prefix
                    raw_observation[f"{joint_name}.pos"] = left_joint_pos[0, SINGLE_ARM_JOINT_NAMES.index(short_name)].item()
                else:
                    short_name = joint_name[6:]  # strip "right_" prefix
                    raw_observation[f"{joint_name}.pos"] = right_joint_pos[0, SINGLE_ARM_JOINT_NAMES.index(short_name)].item()
        ...
        observation = TimedObservation(
            ...
            must_go=True,  # 强制通过 sanity check（动作小时不会被过滤）
        )
    ...
    def _receive_action(self) -> dict:
        ...
        # Debug: log what gRPC actually returned
        data_len = len(actions_chunk.data) if actions_chunk.data else 0
        if data_len == 0:
            print(f"Received `Empty` from policy server, waiting for next call")
            return None
        print(f"Received action data: {data_len} bytes")
        return pickle.loads(actions_chunk.data)

    def get_action(self, observation_dict: dict) -> torch.Tensor:
        if not self.skip_send_observation:
            self._send_observation(observation_dict)
            # 等待 server 把 observation 放入队列
            import time as _time
            _time.sleep(0.05)

        # GetActions 会阻塞等 server 处理完，不重试
        action_chunk = self._receive_action()
        if action_chunk is None:
            self.skip_send_observation = True
            print("GetActions returned Empty — using last action")
