# 智慧工厂仿真平台 · 操作指南（Ubuntu 22.04 + 原生 ROS2 Humble）

> **这份就是发给赛队的 `README.md`**：目标机器是 **Ubuntu 22.04 + 原装 ROS2 Humble（不用 Docker）**。
> 照着做就能把平台已有的全部功能跑起来：
> **起仿真 → 建图 → 2D Goal Pose 导航 → 遥操 → 数采 → 转换 → 训练 → 推理**。
>
> - 开发机（宿主机 + Docker 里跑 ROS2）的那份指南是 **`README1.md`**，两份内容一一对应，
>   **上肢遥操 / 数采 / 转换 / 训练 / 推理五段完全一样**；区别只在"ROS2 跑在容器里还是原生装"。
> - 环境安装见 `INSTALL.md`（一键脚本）+ `../环境依赖查证_Ubuntu22.04_Humble.md`（ROS2 依赖查证）。
> - 过时文档归档在 `docs_archive/`（**发布包里不带**）。**冲突时以本文为准。**

### 项目出处与分工（重要，请先看一眼）

本项目是**基于开源项目二次开发**的成果，不是从零自研的仿真平台。准确的来源链条是：

```
NVIDIA Isaac Sim 6.0.1              ← 仿真引擎本体（不随本仓库分发，由 install_all.sh 安装）
  └─ NVIDIA IsaacLab 3.0.0          ← dependencies/IsaacLab（BSD-3-Clause）
       └─ LightwheelAI/leisaac      ← ★ 本仓库的基础框架（Apache-2.0）
            └─ HuggingFace LeRobot 0.4.2  ← 依赖库：数据集格式与策略生态（Apache-2.0）
```

**本仓库在上游基础上完成的工作**：

- **[@nocorn-ops](https://github.com/nocorn-ops)**：仿真栈 5.1→6.0.1 / 2.3→3.0 迁移与兼容性修复；
  智慧工厂赛项场地与任务（`LeIsaac-SmartFactory-v0`）；ROS2 桥接 / SLAM 建图 / Nav2 导航；
  双臂遥操与数采闭环；ACT 推理可靠性排查；发布与部署工程化
- **[@Nagasakiianno](https://github.com/Nagasakiianno)**：自研移动双臂机器人 URDF/资产导入与集成
  （含自碰撞修复）；HDF5→LeRobot v3 转换工具；双臂 ACT 推理管线与 TCP 动作服务；技术文档

**上游 LeIsaac 自带**（非本项目原创）：任务框架与模板、遥操作设备抽象、录制增强、
策略客户端、单臂任务（`pick_orange` / `lift_cube` / `fold_cloth`）、厨房场景等。

> 详细出处、各上游许可证与逐项分工见 **`CREDITS.md`**。
> 上游 `LICENSE`（Apache-2.0）与 `CITATION.cff` 均按原样保留。
...
| [12](#12-常见问题排查) | 常见问题排查（含"车不动 / 闪退 / 按键没反应"） |
...
- 装完它会写 `reproduce/local.env` 把探测到的路径记下来，之后所有脚本自动使用
  （也就是下一节 §0.2 要你确认的那三样东西，脚本会替你填好）。
- **ROS2 不在其中** —— Humble 要用 apt 装，命令在脚本结尾会打印，也见 `../环境依赖查证_Ubuntu22.04_Humble.md`。
...
### 0.1 分工：都在一台机器上，但**必须开不同的终端**

原生版最大的变化：**没有容器了，所以"隔离"要靠你自己分终端**。

```
终端 A（Isaac Sim）                        终端 B / C / D（ROS2 侧）
────────────────────────                   ────────────────────────────
cd <仓库根>                                 cd <仓库根>
bash reproduce/ros2_chassis_teleop.sh       source /opt/ros/humble/setup.bash
  ├ Isaac Sim（场地+双臂+3相机+2D激光）      ros2 launch scripts/nav2_config/ops/...
  ├ SimBridgeServer（TCP :5560）   ◄──────►  ├ ros2_leisaac_bridge.py
  ├ ChassisController（底盘运动学）           ├ Nav2 / slam_toolbox / RViz / 键盘
  └ 上肢：hold / 主手 / 策略
```

> ★ **两条铁律**：
> 1. **终端 A 里绝对不要 `source /opt/ros/humble/setup.bash`**，也不要在终端 B 里跑
>    `python.sh` / `ros2_chassis_teleop.sh`。两边的 `PYTHONPATH` / `LD_LIBRARY_PATH` 会互相污染
>    （典型症状：Isaac Sim 起不来，或 `import rclpy` 找不到模块）。
> 2. **仿真侧 `SimBridgeServer` 同一时刻只服务一个桥接客户端**。所以任何时刻只能有一个
>    `bridge:=true` 的 launch。多开的表现是"**键盘能按、车不动**"。
>    `ops/` 里主入口（teleop/mapping/navigation）默认 `bridge:=true`，辅助（save_map/reset_sim/auto_drive）
>    默认 `false` —— 别乱改。

### 0.2 确认三样东西的路径（**只做一次**）

因为每台机器安装位置不同，脚本用 `reproduce/leisaac_env.sh` **自动探测**：仓库根（按脚本自身位置）、
Isaac Sim（若干候选目录）、装了 lerobot 的解释器、conda 目录。

先让它把探测结果打出来：

```bash
cd <你的仓库根目录>          # 例如 ~/leisaac 或 /opt/leisaac
...
```
[env] 仓库     REPO=/home/xxx/leisaac
[env] Isaac Sim ISAACSIM_DIR=/home/xxx/isaac-sim-6.0.1
[env] lerobot   LR_PY=/home/xxx/miniconda3/envs/leisaac-lerobot-sim/bin/python
```

**任何一项是「（未找到）」就写 `reproduce/local.env` 覆盖**（不用改任何源码）：

```bash
cp reproduce/local.env.example reproduce/local.env
# 然后按注释填（三行，填成你机器上的真实路径）：
#   ISAACSIM_DIR=/你的/isaac-sim-6.0.1
#   LEISAAC_LR_PY=/你的/miniconda3/envs/你的lerobot环境/bin/python
#   CONDA_DIR=/你的/miniconda3
```

也可以临时用环境变量（不改文件）：`ISAACSIM_DIR=/你的/isaac-sim-6.0.1 bash reproduce/xxx.sh`。

**下面所有命令里的 `<仓库根>` 都指这个 `REPO`。** 为方便复制粘贴，每个终端先设好变量：

```bash
cd <仓库根>
export REPO="$PWD"
export ISAACSIM="$ISAACSIM_DIR"      # 上面探测/填写的结果
export LR=<你的 lerobot 解释器>       # 例如 ~/miniconda3/envs/leisaac-lerobot-sim/bin/python
```

### 0.3 本机前提自检（30 秒）

```bash
# ① GPU 与驱动（Isaac Sim 6.0 要求 Linux 驱动 ≥ 580.95.05）
nvidia-smi --query-gpu=name,memory.total,driver_version --format=csv

# ② ROS2 Humble 与需要的包
source /opt/ros/humble/setup.bash
ros2 pkg list | grep -E "^(nav2_bringup|slam_toolbox|rviz2|teleop_twist_keyboard)$"
python3 -c "import rclpy; print('rclpy OK')"

# ③ 主手串口权限（要用真主手时；没有主手可跳过）
ls -l /dev/ttyACM* 2>/dev/null
groups | grep -q dialout && echo "dialout OK" || echo "⚠️ 需要 sudo usermod -aG dialout $USER 后重新登录"
```

> 官方最低 GPU 是 RTX 4080 / 16 GB 显存；**RTX 40 系（Ada）可以用，不用改代码**。
> 显存小于 16 GB 时建议全程 `--headless`、必要时不开 RViz。
> ⚠️ 另：**Ubuntu 22.04 自带 `libxml2.so.2`，不需要 `reproduce/compat_libs_isaac6/`**
> （那是给 26.04 之类新系统准备的旧 ABI 库）—— 启动脚本会检测它是否存在，不存在就自动跳过。
...
`clients=True` 表示 ROS2 侧的桥接客户端连上了（第 3 节才连）—— 原生版就是"另一个终端里跑起来了"。
...
| 通道 | 怎么"打开" | 谁在驱动 |
|---|---|---|
| **底盘** | 另一个 ROS2 终端里跑键控（`ops/teleop.launch.py`）或 Nav2 发目标 | ROS2 `/cmd_vel`，**随时可用** |
...
# 办法 2：底盘本来就自由 —— 另一个 ROS2 终端里用键控/Nav2 开过去，停稳后再操作上肢
...
- ⚠️ 采数据中途**别用"复位"节点**（`reset_sim`）：那会把底盘送回起点

#### 运行中切换（**仿真全程不用重启**）—— 你要的"打开/关掉某个程序"就是这个

上肢由谁驱动、要不要录制，**都可以在仿真跑着的时候从 ROS2 终端切**（各是一条命令）：
...
**对应的完整流程**（仿真开一次，ROS2 终端里按需开关程序）：

| 步骤 | 终端 A（Isaac Sim，全程不动） | 终端 B（ROS2，按需开关程序） |
|---|---|---|
...
| `--record` / `--no_record` | **开** | recorder 一直挂着（按 N / ROS2 终端指令才真正写）；自动命名落在 `datasets/sessions/`；纯建图会话可 `--no_record` |
...
bash reproduce/stop_all.sh --containers-stopped   # 连已退出的ROS2 终端也删
```
...
原生 Humble **不需要ROS2 终端**：直接在本机开一个**新终端**，`source` 一次环境即可：
...
> 之后所有 `ros2 launch …` 都在这个终端里敲。
> ⚠️ **这个终端里不要跑 `bash reproduce/ros2_chassis_teleop.sh`**（那是终端 A 的事）——
> 两边的 `PYTHONPATH` / `LD_LIBRARY_PATH` 会互相污染（典型症状：Isaac Sim 起不来，或 `import rclpy` 找不到）。
> 需要第二个 ROS2 终端：**直接再开一个终端、再 `source /opt/ros/humble/setup.bash` 一次**即可
>（原生版没有容器，"多终端"就是这么开的）。
...
走完**另开两个终端**（各自 `source /opt/ros/humble/setup.bash` 一次）存图 + 归位：

```bash
# 终端 C：存图
ros2 launch scripts/nav2_config/ops/save_map.launch.py
# 终端 D：归位（底盘瞬移回起点，不用重启仿真）
...
```bash
# 一个 ROS2 终端里（auto 模式强制关掉键盘，不需要 tty）
ros2 launch scripts/nav2_config/ops/mapping.launch.py auto:=true
```

约 150 秒，全自动跑完并退出（Ctrl+C 之前它会自己结束）。实测：场地扫出 **4.02×3.03 m** 地图、空闲约 87%。
...
上肢的遥操**都在仿真进程里**（宿主机），不需要ROS2 终端。共同规则：
...
`collect.sh` 开头的提示、仿真终端启动时的 `[自检]` 段、以及ROS2 终端发 `record=begin` 时都会把这两步再念一遍。
...
| 现象 | 原因 / 处理 |
|---|---|
| **Isaac Sim 起不来 / `import rclpy` 失败** | 同一个终端里既 `source` 了 ROS2 又跑了 `python.sh`。**两边必须分终端**（§0.1 铁律 1） |
| `conda activate` 之后 `ros2` 命令找不到 | conda 的 `python3` 抢了系统 python。`conda deactivate` 后再 `source /opt/ros/humble/setup.bash` |
| 主手读不到 / `Permission denied` | `sudo usermod -aG dialout $USER` 然后**重新登录**；`ls -l /dev/ttyACM*` 看权限 |
| RViz 起不来 / GLX 报错 | 原生版不需要 `--gpus all`；查驱动（`nvidia-smi`）与 `DISPLAY` |
| 局域网里别的机器也能看到这些话题 | 可选隔离：所有 ROS2 终端都 `export ROS_DOMAIN_ID=42` + `export ROS_LOCALHOST_ONLY=1`（本机自用推荐） |
| `reproduce/compat_libs_isaac6` 需不需要 | **22.04 不需要**（自带 `libxml2.so.2`）。启动脚本只在库文件真实存在时才加，会自动跳过 |

### 12.2 平台通用（建图/导航/遥操/数采）

| 现象 | 原因 / 处理 |
|---|---|
| **键盘终端每按一个键就刷日志** | **已修**：桥接客户端原来每收到一条 `cmd_vel` 就打一行（键盘 10Hz 发布 → 每秒 10 行）。现在那个终端只有键位表；`currently: speed/turn` 只在你按 `q/z/w/x/e/c` 调速度时出现 |
| **键盘能按、车不动（日志里 cmd 非零）** | ① 开了激光急停（`LiDAR急停=0.xx`）：场地**角落**里前向 ±40° 锥内 0.70m 就是围栏，`0.45` 会直接禁止前进 → 去掉参数或降到 0.25；② 被接触保险杠拦住（日志 `⛔撞到东西，已停`）→ 后退/转向即可 |
| **键盘能按、车不动（日志里 cmd 全是 0）** | 起了**两个桥接客户端** —— 仿真侧会**明确拒绝第二个**并报出来（以前静默排队）。把多余的关掉（含 `ros2_mapping.sh`/`ros2_navigation.sh` 起的）；最稳是**再开一个终端**跑辅助入口 |
| **"打开仿真闪退"（窗口开了又退出）** | 看终端最后几行。最常见是**录制文件已存在**（同一分钟内起两次会撞名，已修：精确到秒 + 自动加 `_2` 后缀）。显式 `--dataset_file` 撞名会**启动前 0.2s** 报错并给三个选项（`--resume` / 换名 / `--no_record`） |
| **起第二个仿真被拦住** | 这是**故意的**（单实例锁 `/tmp/leisaac_sim.lock` + 桥接端口预检）。`bash reproduce/stop_all.sh --status` 看谁在跑；确实要并行就 `rm /tmp/leisaac_sim.lock`（会抢显存） |
| **机器人抖动/画龙/走不动** | `/cmd_vel` 上**多个发布者**（键控 + Nav2 同时开）。桥接客户端会警告并列出节点名；手动遥控时先把 Nav2 的 `controller_server` 停掉 |
| 键盘报 `termios.error: (25, ...)` | 这个终端不是交互式 TTY（脚本/nohup 里跑）。**原生版就用真终端** |
| 存图报"等不到 `/slam_toolbox/save_map`" | 建图进程已退出，或不在同一个 ROS2 环境（同一台机器上就是同一环境） |
| 存图报"还没收到地图" | 机器人一步没走 —— 没扫到东西 slam 就不出图 |
| **导航加载了错的地图** | 数字名优先：`map` 留空会选数字最大的 `1.yaml`。**显式写 `map:=arena`**，并看日志 `[nav] 使用地图: ...` |
...
| 掰主手不动 | **没按 `B`**；或主手与仿真的姿势差太多（>20°）。按 `R`/`N` 之后要**再按一次 `B`** |
...
| 想了解 | 看 |
|---|---|
| **本机全流程操作**（宿主机 Isaac Sim + Docker ROS2，本文） | `README.md` |
| **本机全流程操作**（Ubuntu 22.04 原生 Humble，**本文**） | `README.md` |
